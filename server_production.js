// ===========================================
// BAZAR EL ROMERO - SERVIDOR DE PRODUCCIÓN
// ===========================================
...
// Configurar CORS dinámico basado en ALLOWED_ORIGINS
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    const msg = '❌ CORS: Origen no permitido';
...
if (!JWT_SECRET) {
  console.error('❌ JWT_SECRET es obligatorio. Configúralo en .env');
  process.exit(1);
}
...
function validateProduct(product) {
  ...
  if (product.img && typeof product.img === 'string' && !product.img.startsWith('data:') && !product.img.startsWith('http://') && !product.img.startsWith('https://') && !product.img.startsWith('/images/')) {
...
app.delete('/products/:id/hard', authenticate, (req, res) => {
  const id = Number(req.params.id);
  const data = readData();
  const idx = data.findIndex(x => x.id === id);
  if (idx === -1) return res.status(404).json({ error: 'Not found' });
...
// Configurar credenciales de administrador desde variables de entorno
...
function initializeAdminUser() {
  if (!fs.existsSync(USERS_FILE)) {
    ...
    const hashed = bcrypt.hashSync(ADMIN_PASSWORD, 8);
    const adminUser = {
      id: 1,
...
app.listen(PORT, () => {
  ...
  console.log(`🔐 Panel Admin: http://localhost:${PORT}/cargar produstos.html`);
  ...
  if (!process.env.JWT_SECRET || process.env.JWT_SECRET === 'your-super-secret-jwt-key-change-this') {
    console.log('⚠️  IMPORTANTE: Configura JWT_SECRET en .env para producción');
  } else {
    console.log('✅ JWT_SECRET configurado correctamente');
  }
  
  if (!process.env.ALLOWED_ORIGINS) {
    console.log('⚠️  CONFIGURACIÓN: Considera configurar ALLOWED_ORIGINS para mayor seguridad');
  } else {
    console.log('✅ CORS configurado para:', allowedOrigins.join(', '));
  }
  
  console.log('💡 Credenciales de administrador (cambiar en producción):');
  console.log('   👤 Usuario:', ADMIN_USERNAME);
